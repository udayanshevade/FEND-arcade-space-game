...
// loads audio similar to images
  function _loadAudio(url) {
    if (resourceCache[url]) {

      /* If this URL has been previously loaded it will exist within
       * our resourceCache array. Just return that file rather
    ...
    // this URL isn't cached yet so load a new resource and cache it
    var audio = new Audio();

    // oncanplaythrough checks if audio has loaded enough to be played
    audio.oncanplaythrough = function() {
    console.log('k');

    // cache and return this file from here on
    resourceCache[url] = audio;

    /* Once the audio is playable and cached,
     * call all of the onReady() callbacks we have defined.
     */
    if (isReady()) {
      readyCallbacks.forEach(function(func) { func(); });
